...
#'   reads from context. One of `"jde"` (Journal of Development Economics),
#'   `"r_journal"` (The R Journal), `"econometrica"`, `"jss"`, `"apsr"`.
...
.qsr_build_report_rmd <- function(template, title, author, journal,
  ...
  # Try to use a real LaTeX template if available
  journal_map <- c(
    "jde"          = "jde_article.tex",
    "r_journal"    = "r_journal_article.tex",
    "econometrica" = "econometrica_article.tex"
  )
  ...
  # Also generate Rmd for HTML/Word output
  sections <- character()
  ...
  if ("models" %in% include) {
    sections <- c(sections, paste0(
      ...
      "# Models stored in QUASAR context\n",
      "models <- .qsr_context$get_models()\n",
      "if (length(models) > 0) {\n",
      "  modelsummary::modelsummary(models, output = 'markdown',\n",
      "    stars = c('*' = 0.1, '**' = 0.05, '***' = 0.01))\n",
      "} else {\n",
      "  cat('No models registered. Use qsr_model() first.\\n')\n",
      "}\n",
      "```\n"
    ))
  }
  ...
  if ("tables" %in% include) {
    sections <- c(sections, paste0(
      "## Tables\n\n",
      "```{r tables, echo=FALSE}\n",
      "tables <- .qsr_context$get_tables()\n",
      "for (tbl in tables) print(tbl)\n",
      "```\n"
    ))
  }
